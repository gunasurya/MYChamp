...
namespace MYChamp.Controller
{
    ...
    public class LoginController : ControllerBase
    {
        ...
        public async Task<ActionResult<int>> OnPostAsync(Login_model login_Model)
        {
            ...
            {
                login_Model.Name= login_Model.Name.TrimStart().TrimEnd();

                var user = _db.signinaccounts.FirstOrDefault(u=>u.EmailId == login_Model.Name
                 );
                List<Register_Model> accounts = _db.signinaccounts.ToList();

                foreach (var account in accounts) 
                {
                    // Perform actions with each account
                    Console.WriteLine($"Account ID: {account.Id},  Email: {account.EmailId.Count()}");
                }
                Console.WriteLine(_db.signinaccounts.ToList());
                Console.WriteLine(user+"userdetails");
                Console.WriteLine(login_Model.Name.TrimStart().TrimEnd().Count());
                Console.WriteLine(login_Model.Password);
